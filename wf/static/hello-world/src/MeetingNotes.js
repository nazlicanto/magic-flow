...
function MeetingNotes() {
    ...
    const handleJiraProcess = async () => {
        ...
        try {
            ...
            // Instead of creating tasks immediately, show preview
            setPreviewTasks(result.generated.tasks);
    ...
    const handlePreviewUpdate = (updatedTasks) => {
    ...
    const handlePreviewConfirm = async (tasksToCreate) => {
        setIsProcessing(true);
        setError(null);
        try {
            const result = await invoke('generateTasks', { 
                description: transcript,
                tasks: tasksToCreate  // Pass the edited tasks
            });
